...
# ---------------------------
EMAIL_SENDER = "mona100975@gmail.com"
EMAIL_PASSWORD = "wdpg dgmp zonk hcdq"
EMAIL_RECEIVER = "Astha23sahu@gmail.com"
...
st.title("🛡️ TERRA WATCH CONNECT")
st.subheader("Live Environment & Threat Intelligence")
...
while True:
    ...
    try:
        ...
        if not df.empty:
            ...
            try:
                if float(latest.get("temperature",0)) > THRESHOLDS["temperature"]:
                    alert_messages.append("🔥 Temperature too high")
            except: pass

            try:
                if float(latest.get("humidity",100)) < THRESHOLDS["humidity_low"]:
                    alert_messages.append("💧 Humidity too low")
            except: pass

            try:
                if int(latest.get("gunfire",0)) == 1:
                    alert_messages.append("💥 Shock detected")
            except: pass
            ...
            with placeholder.container():
                ...
                c1.metric("🌡 Temperature", latest.get("temperature","-"))
                c2.metric("💧 Humidity", latest.get("humidity","-"))
                c3.metric("⏲ Pressure", latest.get("pressure","-"))
                c4.metric("🔆 UV Index", latest.get("uv_index","-"))
                ...
                # Bilaspur base coordinates
                demo_lat = 22.0797
                demo_lon = 82.1391

                # simulate movement
                demo_lat += random.uniform(-0.0006,0.0006)
                demo_lon += random.uniform(-0.0006,0.0006)
                ...
                try:
                    ...
                    if {"gps_lat","gps_lon"}.issubset(df.columns):
                        ...
                        if real_map.empty:
                            ...
                            st.warning("GPS signal lost — showing simulated tracking")
                            ...
                            st.map(fake_map,zoom=13)

                        else:

                            st.success("Live GPS signal received")
                            st.map(real_map,zoom=13)

                    else:

                        st.warning("GPS module offline — simulated location active")
